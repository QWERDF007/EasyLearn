...
- **原文面板可视折叠与全宽铺满**：侧边栏标题旁提供 `#toggle-reader-button` 可视切换按钮。可视状态下为亮蓝底色眼睛图标，支持常规双栏对照；点击切换为不可视状态后，折叠隐藏中栏原文阅读器，右栏结果面板自适应铺满剩余视口空间（`grid-column: 2 / -1`，宽度扩展至 100%），再次点击瞬间恢复双栏对照。

### 4.2 侧边栏任务与五态进度感知
- **环形 SVG 进度条**：外圈包裹高分辨率 SVG 环（`r=15.5`，`stroke-width=2.2`），精确支持 5 种业务状态：
  1. **解析完成**：翡翠绿实心满环（`#10b981`），状态文本如 `✓ 解析完成 · 26页`；
  2. **解析中**：品牌蓝环形进度（`#3860f4`），动态计算 stroke-dashoffset，状态文本如 `⚡ 解析中 72%`；
  3. **上传中**：经典蓝环形进度（`#2563eb`），状态文本如 `↑ 上传中 50%`；
  4. **排队中**：暖橙色顺时针旋转环（`#f97316`，`.is-spinning` 动画），状态文本 `⌛ 排队中`；
  5. **解析失败**：无进度环，红叉状态文本 `✕ 解析失败`，悬浮时呈现重试操作按钮（`.retry-action`）。
...
- **统一交互按钮**：统一使用 26x26px 现代化矢量 SVG 图标按钮，配备 Tooltip 提示，并优化了重试按钮出现时的文字右侧内边距防遮挡。
