...
### 2.1 侧边栏文档与实时进度感知
- **SVG 环形进度条**：通过圆环描边偏移（`stroke-dashoffset`）精确展示实际进度，模拟了当前系统的 5 种典型任务状态：
  - `HOW DO VISION TRANSFORMERS WORK.pdf`：绿色对勾圆环，解析完成 26 页；
  - `Attention Is All You Need.pdf`：品牌蓝环形推进，显示 `解析中 72%`；
  - `Deep Residual Learning.docx`：环形推进，显示 `上传中 50%`；
  - `Diffusion Models Overview.pptx`：环形旋转动画（`doc-ring-spin`），显示 `排队中`；
  - `Vision Benchmark Results.xlsx`：红叉警示，显示 `解析失败`（支持单据重试）。
- **文件类型圆形徽标**：PDF（红）、PPT（橙）、Word（蓝）、Excel（绿）高识别度矢量标牌。
- **统一矢量操作按钮**：收藏五角星（黄金高亮）、删除垃圾桶、刷新重试，尺寸统一为 26x26px 并自带平滑悬浮过渡。

### 2.2 PDF 阅读器与选框高亮
- **浮动阅读工具栏**：支持翻页（◀ ▶）、比例缩放（-、+、100%）、一键重置 1:1、适应宽度与旋转；
